...
@section('content')
    <div class="card">
        <div class="card-header border-bottom d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">Create Regular User</h5>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="user_type" id="user_type" class="form-select" required>
                            <option value="user" {{ old('user_type') == 'user' ? 'selected' : '' }}>Customer (User)
                            </option>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="status" id="status" class="form-select" required
                            onchange="toggleBanReason(this.value)">
                    ...
                    <div class="col-12" id="ban_reason_container"
                        style="display: {{ old('status') === 'banned' ? 'block' : 'none' }};">
                        <label class="form-label" for="ban_reason">Ban Reason</label>
                        <textarea name="ban_reason" id="ban_reason" rows="2"
                            class="form-control @error('ban_reason') is-invalid @enderror" placeholder="Why is this account being banned?">{{ old('ban_reason') }}</textarea>
                        @error('ban_reason')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 mt-4">
                        <h6 class="mb-3 border-bottom pb-2">Assign Roles (Web Guard)</h6>
                        ...
                        @if ($roles->isEmpty())
                            <p class="text-muted italic">No web roles available. Create some in the Roles section first
                                (using 'web' guard).</p>
                        @endif
                    </div>

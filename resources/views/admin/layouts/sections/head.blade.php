...
@php
    ...
    if (empty($adminPageTitle)) {
        $adminPageTitle = trim($__env->yieldContent('header_title', 'Dashboard'));
    }
@endphp
@php
    $siteTitle = \App\Models\Setting::value('site_name', config('app.name', 'Laravel'));
    $rawFavicon = trim((string) \App\Models\Setting::value('site_logo', ''));
    $favicon = $rawFavicon ? $rawFavicon : asset('backend/assets/img/favicon/favicon.ico');
    if (! $rawFavicon) {
...
<title>{{ $adminPageTitle }} | {{ $siteTitle }}</title>
...
<link rel="stylesheet" href="{{ asset('backend/assets/vendor/css/rtl/theme-bordered.css') }}"
...
<link rel="stylesheet" href="{{ asset('backend/assets/vendor/libs/toastr/build/toastr.min.css') }}" />
...
<link rel="stylesheet" href="{{ asset('backend/assets/vendor/libs/animate-on-scroll/animate-on-scroll.css') }}" />

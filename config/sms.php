...
$defaultProviderFromEnv = trim((string) env('SMS_DEFAULT_PROVIDER', SMSNoc::class), "\"'");
$defaultProviderFromEnv = str_replace('\\\\', '\\', $defaultProviderFromEnv);
...
if (! class_exists($defaultProviderFromEnv) && isset($smsProviderAliases[$defaultProviderFromEnv])) {
    $defaultProviderFromEnv = $smsProviderAliases[$defaultProviderFromEnv];
}

if (! class_exists($defaultProviderFromEnv)) {
    $providerWithNamespace = "Xenon\\LaravelBDSms\\Provider\\{$defaultProviderFromEnv}";
    if (class_exists($providerWithNamespace)) {
        $defaultProviderFromEnv = $providerWithNamespace;
    }
}

$resolvedDefaultSmsProvider = class_exists($defaultProviderFromEnv) ? $defaultProviderFromEnv : SMSNoc::class;
...
return [
     ...
     *-----------------------------------------------------------------------------------------------
     ...
     |-----------------------------------------------------------------------------------------------
     */
    'default_provider' => $resolvedDefaultSmsProvider,
    /*
     *---------------------------------------------------------------------------------------------------------------
